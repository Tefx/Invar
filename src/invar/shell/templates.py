...
def _get_mcp_command() -> dict[str, object]:
    """
    Detect the best way to run the Invar MCP server.

    Priority:
    1. uvx (if uv is available) - works regardless of venv
    2. sys.executable - uses the Python that ran invar init
    ...
    # Fallback: use the Python that ran invar init
    return {
...
def configure_mcp_server(path: Path, console) -> Result[list[str], str]:
    ...
    # Detect best MCP command (uvx preferred, sys.executable fallback)
    mcp_config = _get_mcp_command()
    mcp_config["name"] = "invar"
...
### Recommended: uvx (if you have uv installed)

uvx runs invar-mcp without requiring manual venv setup:
...
If you don't have uv, use the Python path from your venv:
...
Replace `/path/to/your/.venv/bin/python` with the output of:
```bash
...
Use the config from `.invar/mcp-server.json` (auto-generated by `invar init`).

## Testing

```bash
# With uv
uvx --from python-invar[mcp] invar-mcp

# Without uv (from activated venv)
python -m invar.mcp
```
