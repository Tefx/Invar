"""Helpers for dead parameter detection.

Shared pure helpers extracted from dead_param.py to keep core files
within size policy while preserving behavior.
"""
...
def is_protocol_base(base: ast.expr) -> bool:
    ...
    if isinstance(base, ast.Subscript):
        return is_protocol_base(base.value)
    return False
...
def collect_protocol_method_names(tree: ast.AST, protocol_classes: set[str]) -> set[str]:
...
@post(lambda result: all(isinstance(name, str) for name in result))
def collect_protocol_classes(tree: ast.AST) -> set[str]:
    protocol_classes: set[str] = {
        node.name
        for node in ast.walk(tree)
        if isinstance(node, ast.ClassDef) and any(is_protocol_base(base) for base in node.bases)
    }
...
@post(lambda result: isinstance(result, bool))
def is_exempt_decorated_function(node: ast.FunctionDef | ast.AsyncFunctionDef) -> bool:
...
def iter_checked_params(args: ast.arguments) -> list[tuple[str, ast.arg]]:
...
@post(lambda result: isinstance(result, bool))
def is_config_shape_param(function_name: str, param_name: str, arg_node: ast.arg) -> bool:
    """Return True for checker-signature config parameters."""
    if param_name != "config":
        return False
    if not function_name.startswith("check_"):
        return False
    annotation = annotation_name(arg_node.annotation)
...
def collect_names_from_node(node: ast.AST, used_names: set[str]) -> None:
    ...
    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
        for decorator in node.decorator_list:
            collect_names_from_node(decorator, used_names)

        for default in node.args.defaults:
            collect_names_from_node(default, used_names)

        for kw_default in node.args.kw_defaults:
            if kw_default is not None:
                collect_names_from_node(kw_default, used_names)
        ...
        for arg_node in [
            ...
            *node.args.kwonlyargs,
            ...
            if arg_node is not None and arg_node.annotation is not None:
                collect_names_from_node(arg_node.annotation, used_names)

        if node.returns is not None:
            collect_names_from_node(node.returns, used_names)
        return

    if isinstance(node, ast.ClassDef):
        for base in node.bases:
            collect_names_from_node(base, used_names)
        for keyword in node.keywords:
            collect_names_from_node(keyword.value, used_names)
        for decorator in node.decorator_list:
            collect_names_from_node(decorator, used_names)
        return

    for child in ast.iter_child_nodes(node):
        collect_names_from_node(child, used_names)
...
def collect_used_names(node: ast.FunctionDef | ast.AsyncFunctionDef) -> set[str]:
    ...
    for stmt in node.body:
        if isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef)):
            collect_names_from_node(stmt, used_names)
            for nested_stmt in stmt.body:
                collect_names_from_node(nested_stmt, used_names)
            continue
        collect_names_from_node(stmt, used_names)
    return used_names
...
@post(lambda result: result is None or isinstance(result, str))
def call_target_name(node: ast.AST) -> str | None:
    if isinstance(node, ast.Name):
        return node.id
    if isinstance(node, ast.Attribute):
        parent = call_target_name(node.value)
...
@post(lambda result: isinstance(result, bool))
def is_callback_registrar(call_name: str) -> bool:
...
@pre(lambda tree: isinstance(tree, ast.AST))
@post(lambda result: all(isinstance(name, str) for name in result))
def collect_registered_callback_names(tree: ast.AST) -> set[str]:
    ...
    for node in ast.walk(tree):
        ...
        is_registrar = call_name is not None and is_callback_registrar(call_name)
...
@post(lambda result: isinstance(result, bool))
def has_framework_signature_exemption(
    node: ast.FunctionDef | ast.AsyncFunctionDef,
    param_name: str,
    param_node: ast.arg,
    ...
    if param_name == "raw" and any(
        decorator_name(decorator) == "contextmanager" for decorator in node.decorator_list
    ):
        current: ast.AST = node
        while current in parent_map:
            current = parent_map[current]
