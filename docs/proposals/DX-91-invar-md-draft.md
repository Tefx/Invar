# DX-91: INVAR.md Agent Semantic Spec Draft (v3)

Target: ~70-120 lines. This is not a human reference manual. It is part of the active instruction surface agents consume after `CLAUDE.md`.
...
Use the quick tool list in `CLAUDE.md` for command lookup. Keep `INVAR.md` focused on durable rules and repair patterns.
```

Notes for DX-91:
- `INVAR.md` is an agent semantic spec, not a tutorial.
- It must not reference `.invar/examples/` or deleted workflow files.
- Guard diagnostics should link to stable `INVAR.md` sections for exact repair guidance.
