...
@pre(lambda max_length: isinstance(max_length, int) and max_length > 0)
...
@pre(lambda dt: isinstance(dt, datetime))
@post(lambda result: isinstance(result, str))
...
@pre(lambda email: isinstance(email, str))
...
def is_valid_email(email: str) -> bool:
    ...
    >>> is_valid_email("")
    False
    >>> is_valid_email("user.name+tag@domain.co.uk")
    True
    """
    if not email:
        return False
...
@pre(lambda url: isinstance(url, str))
@post(lambda result: isinstance(result, bool))
...
@pre(lambda b: isinstance(b, (int, float)))
